...
import { TextFile, TextFilePos, TextFileRange } from "./text.js";
...
const ignoreAstNodeProps = new Set(["parentNode", "nextNode", "prevNode"]);

export class AstNodeFixator {
    process(node: AstNode) {
        let props = Object.keys(node);
        props.forEach(prop => {
            if (ignoreAstNodeProps.has(prop))
                return;
            let value = node[prop];
    ...
    processProp(node: AstNode, prop: string, value: any) {
        ...
        if (value instanceof AstNode) {
            ...
            let nodes = value.filter(t => t instanceof AstNode) as AstNode[];
            if (nodes.length == 0)
                return;
            nodes.forEach(t => this.processProp(node, prop, t));
            let prev: AstNode = null;
