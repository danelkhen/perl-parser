...
import { PrecedenceResolver } from "./precedence-resolver.js";
...
export class ExpressionParser extends ParserBase {
    ...
    onResolveExpression: (mbe: UnresolvedExpression, parentNode?: Expression) => Expression;

    resolveExpression(mbe: UnresolvedExpression, parentNode?: Expression): Expression {
        if (mbe.nodes.length == 1) {
            let node = mbe.nodes[0];
            if (node instanceof Expression) {
                if (node instanceof UnresolvedExpression)
                    return this.resolveExpression(node, parentNode);
                return node;
            }
        }
        if (this.onResolveExpression)
            return this.onResolveExpression(mbe, parentNode);
        return mbe;
        //TODO:
    ...
    parseExpression(parentNode?: Expression): Expression {
        ...
        if (mbe != null && mbe.nodes.length == 0)
            return null;
    ...
    parseNonParenthesizedList(parentNode?: Expression): NonParenthesizedList {
        ...
        while (this.token != null) {
            ...
            if (!this.token.isAny([TokenTypes.comma, TokenTypes.fatComma]))
                break;
            let comma = this.create(Operator);
            comma.token = this.token;
            node.itemsSeparators.push(comma);
            this.nextNonWhitespaceToken(comma);
        }
        return node;
    }
