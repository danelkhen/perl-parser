...
var jQueryHelper = (function () {
    ...
    jQueryHelper.parseSelector = function (s) {
        var sizzle = jQuery.find;
        var groups = sizzle.tokenize(s);
        return groups;
    };
    jQueryHelper.createElementFromSelector = function (selector) {
        var nodes = jQueryHelper.parseSelector(selector);
        return jQueryHelper.createElementFromSelectorNode(nodes[0]);
    };
    jQueryHelper.createElementFromSelectorNode = function (node) {
        var tagName = "div";
        ...
        var isSvg = jQueryHelper._svgElements[tagName];
        var el;
        if (isSvg)
    ...
    jQueryHelper.getOrAppendChildBySelector = function (parentEl, selector, options) {
        ...
        if (index < total) {
            var selectorNodes = jQueryHelper.parseSelector(selector);
            if (selectorNodes.length != 1)
                throw new Error();
            var selectorNode = selectorNodes[0];
            while (index < total) {
                var dataItem = list != null ? list[index] : null;
                var child = create ? create(dataItem, index) : jQueryHelper.createElementFromSelectorNode(selectorNode);
                var childEl = child[0];
...
function jQueryHelper2() {
    ...
    function ofDataItem(obj) {
        var mapping = this._zip.mappings.firstEq("obj", obj);
        if (mapping == null)
            return null;
